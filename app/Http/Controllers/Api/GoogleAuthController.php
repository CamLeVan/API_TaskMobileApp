...
class GoogleAuthController extends Controller
{
    ...
    {
        try {
            Log::info('Google sign in attempt from Android');

            $request->validate([
                'id_token' => 'required|string'
            ]);

            // For Android, we need to manually verify the token
            // Get user info from token payload (JWT)
            ...
            // Create a Google user object from the payload
            $googleUser = (object) [
                'id' => $payload['sub'],
                'email' => $payload['email'],
