...
Để nhận các sự kiện thời gian thực, client cần kết nối đến WebSocket server:

```text
wss://api.yourdomain.com/ws
```

Khi kết nối, client cần gửi token xác thực qua query parameter:

```text
wss://api.yourdomain.com/ws?token=1|laravel_sanctum_token_hash
```

Sau khi kết nối thành công, client cần đăng ký kênh cho từng nhóm:
...
```json
{
  "event": "team.chat.message.created",
  "data": {
    "id": 125,
    "team_id": 1,
    "user_id": 1,
...
### 2. Tin nhắn được chỉnh sửa

```json
{
  "event": "team.chat.message.updated",
  "data": {
    "id": 124,
    "team_id": 1,
...
### 5. Trạng thái đang nhập

```json
{
  "event": "team.chat.typing",
  "data": {
    "team_id": 1,
    "user_id": 1,
    "is_typing": true,
    "timestamp": "2025-05-10T09:07:00.000000Z"
  }
}
```
...
```json
{
  "event": "team.chat.message.reaction",
  "data": {
    "message_id": 124,
    "user_id": 1,
    "reaction": "👍",
...
### 11. Tải lên tệp đính kèm

**Endpoint:** `POST /api/teams/{team_id}/chat/attachments`

**Mô tả:** Tải lên tệp đính kèm trước khi gửi tin nhắn.
...
Sử dụng `multipart/form-data` để tải lên tệp.

- `file` (required): Tệp cần tải lên
- `type` (optional): Loại tệp (image, document, video, audio)
...
| 413 | Payload Too Large | Kích thước tệp đính kèm vượt quá giới hạn |
...
```kotlin
// Kotlin với Retrofit và OkHttp WebSocket
...
// WebSocket
class ChatWebSocketClient(private val token: String) {
    private val client = OkHttpClient.Builder()
        .readTimeout(0, TimeUnit.MILLISECONDS) // Vô thời hạn cho WebSocket
        .build()
    ...
    fun connect() {
        ...
        webSocket = client.newWebSocket(request, object : WebSocketListener() {
            override fun onOpen(webSocket: WebSocket, response: Response) {
                // Đăng ký kênh
                webSocket.send("""{"event":"subscribe","channel":"team.1.chat"}""")
            }
...
data class Message(
    ...
    val user: User,
    val read_status: List<ReadStatus>,
