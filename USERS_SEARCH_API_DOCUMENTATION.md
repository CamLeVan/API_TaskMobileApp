...
**Lưu ý quan trọng:** API này sẽ trả về tất cả người dùng phù hợp với từ khóa tìm kiếm, bao gồm cả người dùng hiện tại.
...
### Tìm kiếm người dùng theo từ khóa chung (tương thích với ứng dụng Android hiện tại)

```text
GET /api/users/search?query=nguyen
```
...
```text
GET /api/users/search?query=nguyen&exclude_team=5
```

### Tìm kiếm với số lượng kết quả tùy chỉnh

```text
GET /api/users/search?query=nguyen&per_page=10
```
